...
describe("Put/booking/:bookingId", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 when bookingId sent not exists", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
    ...
    it("should respond with status 403 when bookingId sent is not the same of bookingID found by userId", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
    ...
    it("should respond with status 200 when booking is updated", async () => {
      ...
      const createdHotel = await createHotel();
      const createdRoom = await createRoomWithCapacity(createdHotel.id, 3);
      const createdRoom2 = await createRoomWithCapacity(createdHotel.id, 3);
      const booking = await createBooking(user.id, createdRoom.id);
